...
class TelemetryServer {
    constructor(ip,port){
        ...
        setInterval(function() {
            $.getJSON(telemetryInstance.address+"/live", function(data){
                ...
                for (let i = 0; i < telemetryInstance.dataCallbacks.length; i++){
            ...
            $.getJSON(telemetryInstance.address+"/stats", function(data){
                ...
                for (let i = 0; i < telemetryInstance.statsCallbacks.length; i++){
            ...
            $.getJSON(telemetryInstance.address+"/alarms", function(data){
                ...
                for (let i = 0; i < telemetryInstance.alarmCallbacks.length; i++){
    ...
    getNumAlarms(){
        let numAlarms = 0;
        for (let alarmKey in this.alarms) {
            if (this.alarms.hasOwnProperty(alarmKey)) {
                if (this.alarms[alarmKey].state){
                    numAlarms++;
                }
            }
        }
    ...
    getNumValidDataPoints(){
        let numDataPoints = 0;
        for (let dataPointKey in this.lastDataPoints.telemetry) {
        ...
        if (!this.isPaused){
            ...
            if (source == "telemetry"){
                if (key in this.dataPointCallbacks){
                    for (let i = 0; i < this.dataPointCallbacks[key].length; i++){
...
export const Telemetry = new TelemetryServer("localhost", "8727");
